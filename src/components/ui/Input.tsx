...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className, label, error, leftElement, rightElement, labelClassName, description, variant = 'default', ...props }, ref) => {
    const variants = {
      default: 'bg-white text-slate-900 border-slate-200 placeholder:text-slate-400 focus-visible:ring-primary-500',
      glass: 'bg-white/5 text-white border-white/10 placeholder:text-white/20 focus-visible:ring-primary-500/50',
    };

    return (
      <div className="w-full space-y-1.5">
        {label && (
          <label className={cn('text-sm font-bold text-white/80', labelClassName)}>
            {label}
          </label>
        )}
